...
唯一跳过项：`tests/test_hardening_acceptance.py` 的真实 CrewAI + 自托管 LLM 集成测试。
...
本证据证明当前工作树在独立 PostgreSQL 17 实例上的 RLS、租户隔离、并发回调、checkpoint、审计和恢复一致性测试通过。它不证明目标生产服务器、PostgreSQL HA、真实租户、真实模型、真实 CrewAI 或真实资金渠道已经就绪。
...
- 真实集成测试当前失败：端点返回普通文本而非工具调用，CrewAI 选择 `escalate_ticket`，未产生 `query_order` 工具调用。
- 端点源码 `src/llm/self_hosted_server.py` 明确复用 `MockLLM` 规则引擎，因此该端点属于**自托管代表性/Mock 引擎**，不是真实权重模型。

结论：本轮完成了真实 CrewAI 运行环境和本地端点连通性验证，但**真实权重模型工具调用能力仍 BLOCKED**，唯一真实 CrewAI 集成测试不能标记通过。适配层已修复 LiteLLM 对 OpenAI-compatible 模型 provider 前缀的要求（`openai/<model>`）。
