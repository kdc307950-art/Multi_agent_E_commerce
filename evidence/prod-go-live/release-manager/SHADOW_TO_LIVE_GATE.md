...
- 对应门禁（`GO_NO_GO.md`：G12 = `✅ 有序`、G13 = `❌ BLOCKED-需业务方`、G14 = `❌ 待切 live 后`，见 GO_NO_GO.md §二）：本流程**已定义** shadow→live 受控门控；但**真实租户书面确认（G13）与 7 天观察（G14）**尚未完成（外部输入未到，定稿为阻塞），故**当前不得切 live**。
