...
> **单源**：与 `deploy/records/7DAY_ACCEPTANCE_TRACKER.md` 同内容；改一处必改两处。
> **每日执行**：某天的勾选卡见下方「Day N 勾选卡」，逐项填 **✅ 正常(绿) / ❌ 异常(红) / ⬜ 未观测**；每项填**检测依据**（指标/SQL/Loki/审计），
> 触发处置闭环（观察 → 定性 → 止损 → 排查 → 恢复/回滚 → 复核 → 复盘，见 `OPS_RUNBOOK §4`）。只写客观事实；租户明细走受控审计。
...
| 关键告警无未处理 | 各红线告警不命中，命中即闭环 | 见 §3；命中项均已处置且恢复 |

## 2. 8 项每日检查（正常=绿，异常=红；检测依据见 DAILY_CHECKS_RUNBOOK）

| 检查 | 阈值 | 状态 |
|------|------|------|
| 租户越权（§1） | 越权=0 | ⬜ |
| 审批异常（§2） | 失败率≤5%，p95≤30s，越权=0 | ⬜ |
| 重复 operation（§3） | 重复=0 | ⬜ |
| 回调 mismatch（§4） | 异常=0 | ⬜ |
| 延迟（§5） | p95≤30s；端到端<10s | ⬜ |
| 人工介入率（§6） | ≤20% | ⬜ |
| 模型拒答率（§7） | 绕过=0；拒答率≤基线 | ⬜ |
| 失败补偿（§8） | 失败=0 | ⬜ |
...
> ↓ 复制上方勾选卡填 Day 2–Day 7（每日一张），并记录每日备份恢复演练结果（`deploy/drills/record_template.md`，RPO≤15min/RTO≤60min）。
