...
| 边界 | 定义 | 关键验证项 |
|:---:|------|-----------|
| **边界1 单元测试** | `pytest tests/`（内存/SQLite） | 全量实测 **320 passed, 34 skipped, 70 errors**（424 collected，28.85s，EXIT=1；**t1 实测**）。**70 errors 全部**为沙箱清理 `tmp_path` 的 `PermissionError [WinError 5]` **环境权限问题，0 个真失败**（测试逻辑全过），非功能失败。注：`390 passed / 1 skipped` 为 `PRODUCTION_ACCEPTANCE_VERIFICATION` 中**干净环境（非 PG）基线投影**（＝320 + 修好 70 个 tmpdir 环境错误后），本沙箱不可实现，**仅为投影值**，与实测 320/34/70 不矛盾但不混同，**不采信为实测**。 |
