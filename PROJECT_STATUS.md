# 项目状态说明（一页 · 生产候选版 v1.0.0-rc3 · 2026-09-04）
...
| 边界 | 定义 | 关键验证项 |
|:---:|------|-----------|
| **边界1 单元测试** | `pytest tests/`（内存/SQLite） | 全量实测 **390 passed, 34 skipped**（424 collected，54.63s，EXIT=0；**发布基线环境实测**）。34 skipped = **33 项 PostgreSQL 数据面**（无 `DATABASE_URL`）+ **1 项 CrewAI 真实调用链**（`test_hardening_acceptance.py:384`）。此前记录的 `320 passed, 34 skipped, 70 errors` 均为**受限沙箱清理 `tmp_path` 的 `PermissionError [WinError 5]` 环境权限问题**所致（70 errors 均属环境问题、0 个真失败；发布基线环境无此限制），故修正后实测 390/34。 |
...
- ✅ **已提交收口**：阶段一 rc3 发布基线收口提交已纳入（测试默认写临时目录 + 真实 390/34 口径 + uv.lock 不入库），工作区 **clean**（以 `release/v1.0.0-rc3` 为锚）。
- ✅ 重定稿 `GO_NO_GO.md` / `FINAL_ACCEPTANCE.md`，统一四证据边界，消除"已完成/未接入"矛盾（修正 rc1→cd743d3、A8/G9 生产栈健康已证实、迁移版本 4 点表述；**rc3 以基线尖端为锚** 口径）。
- ✅ 统一 `README.md` 生产候选版口径（11→12 份文档、PostgreSQL/RLS 已接入验证、全量 pytest **390 passed / 34 skipped**（**发布基线环境实测**；34 skipped=33 PG 无 DATABASE_URL+1 CrewAI）、放量 NO-GO、rc3 以基线尖端为锚）。
