# 预发布演练记录 — 加密备份恢复（DR-20260904135305）

- 演练编号：DR-20260904135305
- 环境：preview（PostgreSQL，DB=langgraph；备份角色 backup_role 仅只读；恢复用迁移/owner 角色）
- 应用 git 引用：3268a1c
- 备份点快照：2026-09-04T13:53:04+0800（epoch=1788501184）
...
| E4 | 解密恢复（RTO） | PASS | RTO=0.094207s |
| E5 | 快照点后变更排除（marker） | PASS | marker_in_restore=0 |
| E6 | 采样数据保留（TENANT-A/B） | PASS | A=1 B=1 |

## RPO / RTO 实测
- RPO（实测数据丢失窗口）：**0.815516s**；RPO 上界（备份周期最坏情况）：900s（阈值 ≤900s）
- RTO（恢复耗时实测）：**0.094207s**（阈值 ≤3600s）
